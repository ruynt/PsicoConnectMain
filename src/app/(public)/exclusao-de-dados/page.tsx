import Link from "next/link";
import { legalBlocks } from "@/lib/legal/legalContent";
import LegalTabs from "@/components/legal/LegalTabs";
import styles from "../legal/legal.module.css";

function getBlock(id: string) {
  const block = legalBlocks.find((item) => item.id === id);

  if (!block) {
    throw new Error(`Documento legal não encontrado: ${id}`);
  }

  return block;
}

function DocumentContent({ blockId }: { blockId: string }) {
  const block = getBlock(blockId);

  return (
    <section id={block.id} className={styles.documentCard}>
      <header className={styles.documentHeader}>
        <span className={styles.documentNumber}>01</span>

        <div className={styles.documentTitleGroup}>
          <h2>{block.title}</h2>
          <p>{block.summary}</p>
        </div>
      </header>

      <div className={styles.documentSections}>
        {block.content.map((section) => (
          <article key={section.heading} className={styles.textBlock}>
            <h3>{section.heading}</h3>

            {section.paragraphs?.map((paragraph) => (
              <p key={paragraph}>{paragraph}</p>
            ))}

            {section.items && (
              <ul>
                {section.items.map((item) => (
                  <li key={item}>{item}</li>
                ))}
              </ul>
            )}
          </article>
        ))}
      </div>
    </section>
  );
}

export const metadata = {
  title: "Exclusão de Dados | PsicoConnect",
  description:
    "Como solicitar exclusão de conta, dados pessoais e desconexão da integração com Google Calendar no PsicoConnect.",
};

export default function LegalDocumentPage() {
  return (
    <main className={styles.legalPage}>
      <section className={styles.heroSection}>
        <div className={styles.heroInner}>
          <Link href="/" className={styles.backLink}>
            <i className="fa-solid fa-arrow-left" aria-hidden="true" />
            Voltar
          </Link>

          <div className={styles.heroCard}>
            <span className={styles.badge}>Exclusão de Dados</span>

            <h1>Exclusão de Dados</h1>

            <p>Orientações sobre solicitação de exclusão de conta, dados pessoais, revogação de consentimento e desconexão do Google Calendar.</p>

            <span className={styles.updated}>
              Última atualização: Junho de 2026
            </span>
          </div>
        </div>
      </section>

      <section className={styles.contentSection}>
        <div className={styles.singleDocumentInner}>
          <LegalTabs active="exclusao" />

          <DocumentContent blockId="exclusao-dados" />
        </div>
      </section>
    </main>
  );
}
